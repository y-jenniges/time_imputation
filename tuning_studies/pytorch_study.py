...
def suggest_hyperparameters(trial, model_name="mae"):
    ...
    if model_name == "unet":
        ...
        strategy = trial.suggest_categorical("strategy", ["random", "sphere", "transect"])
...
def train_pytorch_single_split(coords_raw, values_raw, model_class, hyps, train_idx, val_idx, test_idx, model_name,
    ...
    cfg.sphere_mask_radius = None if masking is None else masking["sphere_mask_radius"]
    ...
    if model_name == "mastnet":
        graph_provider = GraphProvider(n_neighbours=n_neighbours,
                                       update_every=cfg.graph_update_frequency,
                                       cfg=cfg,
