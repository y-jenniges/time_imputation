...
def suggest_hyperparameters(trial):
    ...
    return {
            ...
            "model": {
                "base_ch": trial.suggest_categorical("base_ch", [32, 64, 128, 256]),
...
def train_unet_single_split(df, model_class, hyps, train_idx, val_idx, test_idx, model_name, split_path, trial_id,
    ...
    base_name = f"model{model_name}_trial{trial_id}_split{split_fname}"
...
def optuna_objective(trial, model_name):
    ...
    for split_i, split_path in enumerate(split_paths):
        ...
        # Train
        results, _, _ = train_unet_single_split(
            df=DATA,
            model_class=OceanUNet,
            hyps=hyp_dict,
            train_idx=train_idx,
            val_idx=val_idx,
            test_idx=test_idx,
